...
const SuasRotinas = () => {
  ...
  const confirmarRemocaoRotina = async () => {
    ...
    try {
      await fetch(`ecobalance-backend.onrender.com/api/rotinas/${rotinaParaDeletar}`, {
        method: 'DELETE',
      });
  ...
  useEffect(() => {
    ...
    const buscarRotinas = async () => {
      try {
        const response = await fetch(`ecobalance-backend.onrender.com/api/rotinas/usuario/${usuario._id}`);
