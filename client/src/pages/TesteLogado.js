...
const TesteLogado = () => {
  ...
  useEffect(() => {
    ...
    const carregarRotinas = async () => {
      try {
        ...
        const response = await fetch(`ecobalance-backend.onrender.com/api/rotinas/usuario/${usuarioLogado._id}`);
        if (response.ok) {
  ...
  const handleFinalizarTeste = async () => {
    try {
      ...
      const response = await fetch("ecobalance-backend.onrender.com/api/testes", {
        method: "POST",
        headers: {
